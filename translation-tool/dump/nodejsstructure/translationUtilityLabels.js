...
async function sourceXMLParse() {
	...
	try{
		let stream = fs.createReadStream('inputs/source_meta/labels/CustomLabels.labels', 'UTF-8');
		stream.pipe(parser);
		...
		let wstream = fs.createWriteStream('C:/BuildHughes/translation-tool/temp/ConsoleLabels.txt','UTF-8');
		...
		wstream.end();
		console.log('Parsed File created ---> translation-tool/temp/ConsoleLabels.txt');
...
var filepath = 'output/Label_Translation_Search_Errors.xls';
...
out.write('Metadata Type\tTranslation Filename\tAPI Name\tValue\tError\tSuggested Fix\n', 'UTF-8');
		...
		fs.readdir('inputs/translation_files/translations', (err, files) => {
		files.forEach(file => {
			fs.readFile('inputs/translation_files/translations/'+file, (err, data) => {
			var j = 0;
