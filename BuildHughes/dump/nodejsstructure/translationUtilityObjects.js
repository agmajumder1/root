...
async function sourceXMLParse() {

	// Label Reading starts in custom labels   //To be updated to read any files
	try{
		fs.readdir('C:/BuildHughes/translation-tool/inputs/source_meta/objects/', (err, files) => {
		files.forEach(file => {
		objname = file.substr(0, file.length-7);
		let stream = fs.createReadStream('C:/BuildHughes/translation-tool/inputs/source_meta/objects/'+file, 'UTF-8');
		stream.pipe(parser);
		...
		*/
		...
		let wstream = fs.createWriteStream('C:/BuildHughes/translation-tool/temp/ParsedObjects.txt','UTF-8');
			...
			wstream.end();
			console.log('Parsed File created ---> C:/BuildHughes/translation-tool/temp/ParsedObjects.txt');
...
async function searchTransValue(){
//	parser.on('finish', function() {
	console.log('Creating Output File ---> C:/BuildHughes/translation-tool/output/ Folder....');
// Label Search starts in translation file

var filepath = 'output/Object_Translation_Search_Errors.xls';
...
out.write('Metadata Type\tTranslation Filename\tAPI Name\tValue\tError\tSuggested Fix\n', 'UTF-8');
		...
		fs.readdir('inputs/translation_files/objectTranslations', (err, files) => {
		files.forEach(file => {
			fs.readFile('inputs/translation_files/objectTranslations/'+file, (err, data) => {
			var j = 0;
				...
				//API name absent in translation.
						...
						var str = 'HelpText\t'+file+'\t'+api+'\t'+val+'\tAPI Name not found in translation file\tPlease add to translation file\n';
						out.write(str, 'UTF-8');
					}
				//Label value commented.
				else if(data.includes(cval)){
						var str = 'HelpText\t'+file+'\t'+api+'\t'+val+'\tValue commented in translation file\tPlease update translation in translation file\n';
						out.write(str, 'UTF-8');
					}
				//label value commented with API name as commented label value.
				else if(data.includes('<!-- '+api+' -->')){
						var str = 'HelpText\t'+file+'\t'+api+'\t'+val+'\tAPI name or Value commented in translation file\tPlease add to translation file\n';
						out.write(str, 'UTF-8');
					}
				}
				});
